...
m.views.Weather = Backbone.View.extend({
    ...
    render: function() {
        var variables = { temperature: this.model.get('temperature'),
        ...
        this.$el[order]('#' + this.options.region).html(this.template(variables));
    ...
    saveLocation: function () {
        this.model.save('manualLocation', this.$location.text());
        this.doneEditingLocation();
    },
    ...
    updateWeather: function() {
        ...
        function getLocation(apiLocation) {
            ...
            // var apiQuery = 'http://query.yahooapis.com/v1/public/yql?q=select%20woeid%2Ccountry%2Cname%20from%20geo.places%20where%20text%3D%22' + encodeURIComponent(apiLocation) + '%22&format=json&diagnostics=true&callback=';
            var apiQuery = 'http://query.yahooapis.com/v1/public/yql?q='+encodeURIComponent('select woeid,name,country from geo.places where text="' + apiLocation + '"')+'&format=json&callback=';
            // console.log(apiQuery);

            $.getJSON(apiQuery, function(r){
                var count = r.query.count, results;
                if (count > 1) {
                    results = r.query.results.place[0];
                } else if (count == 1) {
                    results = r.query.results.place;
                } else {
                ...
                if (!that.model.get('location')) {
                    that.setUnit(results.country.code);
                }

                // HACK: Store country code globally here to
                // use to determine whether or not to show promo
                // images.
                localStorage.country = results.country.code;

                var location = results.name;
        ...
        function getWeather(woeid) {
            ...
            var weatherYQL = 'http://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent('select * from weather.forecast where woeid=' + woeid + ' and u="'+ that.model.get('unit') + '"') + '&format=json&callback=';
